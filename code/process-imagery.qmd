...
color_out <- color_parameters_total |>
  ...
  partition(cl) |>
   mutate(
    result = pmap(list(inpath, transform), ~parallel_color_transforms(.x, .y, do_computation = FALSE, over_write = FALSE)[[1]]),
    outpath = map_chr(result, "outpath"),
    outpath_exists = map_lgl(result, ~ ifelse(is.na(.x$outpath), FALSE, file.exists(.x$outpath))),
    error_message = map_chr(result, "error")
  ) |>  
  #dplyr::select(-result) |>
  collect() 
