...
color_parameters <- file_paths |> 
  cross_join(color_transforms)

color_out <- color_parameters |>
  mutate(inpath_exists = file.exists(inpath)) |>
  rowwise() |>
  mutate(outpath = parallel_color_transforms(inpath, transform, do_computation = FALSE, over_write = FALSE)) |>
  ungroup() |>
...
texture_out <- texture_parameters |>
 ...
 partition(cl) |>
  mutate(outpath = parallel_texture_calculation(inpath, window, statistic, layer,
                                                 do_computation = FALSE, over_write = FALSE)) |>
    collect() |>
  mutate(outpath_exists = file.exists(outpath)) 
texture_out
```
